...
import DropdownPortal from './Dashboard/DropdownPortal';
import CreateModal from './Dashboard/CreateModal';
import DeleteModal from './Dashboard/DeleteModal';
import RenameModal from './Dashboard/RenameModal';
import Notification from './Dashboard/Notification';
import MainContent from './Dashboard/MainContent';
import { fetchMindmaps, createMindmap, renameMindmap, saveMindmap, deleteMindmap } from '../utils/mindmapApi';
...
const Dashboard = ({ user, onLogout }) => {
  ...
  useEffect(() => {
    fetchMindmaps(setLoading, setMindmaps, showNotificationMessage);
  }, []);

  const filteredMindmaps = mindmaps.filter(m => m.name.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      (e) => {
        if (e.key === 's' && (e.metaKey || e.ctrlKey)) {
          ...
          if (showMindmapEditor && currentMindmap) {
            saveMindmap(currentMindmap.id, currentMindmap.name, setMindmaps, showNotificationMessage);
          }
        }
  ...
  return (
      <div
          className={`h-screen w-screen flex transition-colors duration-300 overflow-hidden ${
      ...
      >
        ...
        <MainContent
            isDarkMode={isDarkMode}
            showMindmapEditor={showMindmapEditor}
            currentMindmap={currentMindmap}
            user={user}
            onRename={() => setShowRenameModal({ show: true, mindmap: currentMindmap, newName: currentMindmap?.name || '' })}
            onSave={() => saveMindmap(currentMindmap?.id, currentMindmap?.name, setMindmaps, showNotificationMessage)}
            onShare={() => shareMindmap(currentMindmap)}
        ...
        <CreateModal
            ...
            onCreate={() =>
                createMindmap(newMindmapName, setMindmaps, setCurrentMindmap, setShowCreateModal, setNewMindmapName, showNotificationMessage)
            }
            onCancel={() => {
        ...
        <DeleteModal
            ...
            mindmap={showDeleteModal.mindmap}
            onConfirm={() => {
              deleteMindmap(showDeleteModal.mindmap?.id, setMindmaps, setCurrentMindmap, showNotificationMessage);
              setShowDeleteModal({ show: false, mindmap: null });
            }}
            onCancel={() => setShowDeleteModal({ show: false, mindmap: null })}
        />
        <RenameModal
            isDarkMode={isDarkMode}
            showRenameModal={showRenameModal.show}
            newName={showRenameModal.newName}
            setNewName={(value) => setShowRenameModal(prev => ({ ...prev, newName: value }))}
            onConfirm={() => {
              renameMindmap(showRenameModal.mindmap?.id, showRenameModal.newName, setMindmaps, setCurrentMindmap, showNotificationMessage);
              setShowRenameModal({ show: false, mindmap: null, newName: '' });
            }}
            onCancel={() => setShowRenameModal({ show: false, mindmap: null, newName: '' })}
        />
        <Notification show={showNotification.show} message={showNotification.message} type={showNotification.type} />
        {activeDropdown && (
            <DropdownPortal
                ...
                onSave={(mindmap) => saveMindmap(mindmap.id, mindmap.name, setMindmaps, showNotificationMessage)}
                onDelete={(id) => {
                  setShowDeleteModal({ show: true, mindmap: mindmaps.find(m => m.id === id) });
                  setActiveDropdown(null);
                }}
            />
        )}
