...
export default function Sidebar() {
 ...
 const { load: loadGuests } = useLocalMindmap();
 ...
 const handleRename = (id: string) => addToast(`Chức năng đổi tên đang được phát triển.`, "info");
 const handleDelete = (id: string) => addToast(`Chức năng xoá đang được phát triển.`, "info");
 	 	 	 	 	  ...
 	 	 	 	 	  filteredMindmaps.map((m) => (
 	 	 	 	 	 	  <div key={m.id} className="group relative w-full text-left p-3 rounded-md hover:bg-gray-800 transition-colors flex items-center justify-between">
 	 	 	 	 	 	 	  ...
 	 	 	 	 	 	 	  <div className="opacity-0 group-hover:opacity-100 flex items-center gap-1 pl-2">
 	 	 	 	 	 	 	 	  ...
 	 	 	 	 	 	 	 	  <button onClick={() => handleRename(m.id)} className="p-1.5 rounded hover:bg-gray-700" title="Đổi tên"><Edit size={14}/></button>
